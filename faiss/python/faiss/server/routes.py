...
class EvictedListsResponse(BaseModel):
    """Response model for evicted IVF lists."""
    evicted_lists: List[int] = Field(..., description="List IDs currently evicted to CPU")


class EvictionRequest(BaseModel):
    """Request model for manual IVF list eviction."""
    list_ids: List[int] = Field(..., description="List IDs to evict from GPU to CPU")


class EvictionResponse(BaseModel):
    """Response model for manual IVF list eviction."""
    reclaimed_bytes: Dict[str, int] = Field(
        ..., description="Mapping from list_id to reclaimed bytes"
    )


class LoadRequest(BaseModel):
    """Request model for manual IVF list loading."""
    list_ids: List[int] = Field(..., description="List IDs to load from CPU to GPU")


class LoadResponse(BaseModel):
    """Response model for manual IVF list loading."""
    loaded_bytes: Dict[str, int] = Field(
        ..., description="Mapping from list_id to loaded bytes"
    )


class ClusterStat(BaseModel):
    """Per-IVF-list activation statistics."""
    list_id: int = Field(..., description="IVF list (cluster) id")
    probe_count: int = Field(..., description="Total probe activations")
    load_count: int = Field(..., description="Total load activations")
    last_probe_ts: float = Field(
        ..., description="Last probe time (monotonic, implementation-defined)"
    )


class ClusterStatsResponse(BaseModel):
    """Response model for IVF cluster activation statistics."""
    stats: List[ClusterStat] = Field(
        ..., description="Per-list activation statistics"
    )
...
async def root():
    ...
    return {
        ...
        "endpoints": {
            ...
            "stats": "/api/v1/stats",
            "admin_evicted_lists": "/api/v1/admin/evicted_lists",
            "admin_evict": "/api/v1/admin/evict",
            "admin_load": "/api/v1/admin/load",
            "admin_cluster_stats": "/api/v1/admin/cluster_stats",
        }
    }


@router.get("/admin/evicted_lists", response_model=EvictedListsResponse)
async def get_evicted_lists(
    engine: BaseEngine = Depends(get_engine),
):
    """Return the set of IVF list IDs currently evicted to CPU (if supported)."""
    if not hasattr(engine, "index") or not hasattr(engine.index, "nlist"):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Engine index does not expose IVF lists",
        )
    import faiss  # local import to avoid circular dependencies

    if not hasattr(faiss, "get_evicted_lists"):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="faiss.get_evicted_lists is not available",
        )
    try:
        evicted = faiss.get_evicted_lists(engine.index)
        evicted_ids = [int(x) for x in evicted]
    except Exception as exc:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to fetch evicted IVF lists: {exc}",
        )
    return {"evicted_lists": evicted_ids}


@router.post("/admin/evict", response_model=EvictionResponse)
async def evict_lists(
    request: EvictionRequest,
    engine: BaseEngine = Depends(get_engine),
):
    """Manually evict a collection of IVF lists to CPU (if supported by engine)."""
    if not hasattr(engine, "evict_lists"):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Engine does not expose evict_lists API",
        )
    try:
        reclaimed = engine.evict_lists(request.list_ids)
    except Exception as exc:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Evict lists failed: {exc}",
        )
    reclaimed_str_keys = {str(k): int(v) for k, v in reclaimed.items()}
    return {"reclaimed_bytes": reclaimed_str_keys}


@router.post("/admin/load", response_model=LoadResponse)
async def load_lists(
    request: LoadRequest,
    engine: BaseEngine = Depends(get_engine),
):
    """Manually load a collection of IVF lists back to GPU (if supported)."""
    if not hasattr(engine, "load_lists"):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Engine does not expose load_lists API",
        )
    try:
        loaded = engine.load_lists(request.list_ids)
    except Exception as exc:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Load lists failed: {exc}",
        )
    loaded_str_keys = {str(k): int(v) for k, v in loaded.items()}
    return {"loaded_bytes": loaded_str_keys}


@router.get("/admin/cluster_stats", response_model=ClusterStatsResponse)
async def get_cluster_stats(
    engine: BaseEngine = Depends(get_engine),
):
    """Return backend-provided IVF cluster activation statistics (if available)."""
    if not hasattr(engine, "get_cluster_stats"):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Engine does not expose get_cluster_stats API",
        )
    try:
        stats_dict = engine.get_cluster_stats()
    except Exception as exc:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to fetch cluster stats: {exc}",
        )

    stats_list: List[Dict[str, Any]] = []
    for list_id, s in (stats_dict or {}).items():
        stats_list.append(
            {
                "list_id": int(list_id),
                "probe_count": int(s.get("probe_count", 0)),
                "load_count": int(s.get("load_count", 0)),
                "last_probe_ts": float(s.get("last_probe_ts", 0)),
            }
        )
