...
class PriorityScheduler(BaseScheduler):
    """Scheduler that prioritizes requests based on source_id priority.

    Higher priority values are served first. When multiple sources share the
    same priority, we fall back to FIFO ordering based on enqueue_time.
    """
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(max_batch_size, max_wait_ms, step_interval_ms)
        self._queues: Dict[str, Deque[SchedulerRequest]] = {}
        self._source_priorities: Dict[str, int] = dict(source_priorities or {})
        self._default_priority = int(default_priority)

    def _get_priority(self, source_id: str) -> int:
        return int(self._source_priorities.get(source_id, self._default_priority))

    def add_request(self, request: SchedulerRequest) -> None:
        if request is None:
            raise ValueError("request must not be None")
        if request.source_id is None or request.source_id == "":
            raise ValueError("request.source_id must be a non-empty string")
        with self._lock:
            if request.source_id not in self._queues:
                self._queues[request.source_id] = deque()
            self._queues[request.source_id].append(request)
    ...
    def _pop_batch(self, now: float, max_count: int) -> List[SchedulerRequest]:
        ...
        batch: List[SchedulerRequest] = []
        with self._lock:
            while self._queues and len(batch) < max_count:
                best_source_id: Optional[str] = None
                best_priority: Optional[int] = None
                best_enqueue: Optional[float] = None

                for source_id, queue in self._queues.items():
                    if not queue:
                        continue
                    prio = self._get_priority(source_id)
                    ts = queue[0].enqueue_time
                    ...
                    if prio > best_priority:
                        ...
                        best_priority = prio
                        best_enqueue = ts
                    elif prio == best_priority and ts < best_enqueue:
                        best_source_id = source_id
                        best_priority = prio
                        best_enqueue = ts
                ...
                queue = self._queues.get(best_source_id)
                if not queue:
                    # Queue may have been emptied between selection and access
                    self._queues.pop(best_source_id, None)
                    continue
